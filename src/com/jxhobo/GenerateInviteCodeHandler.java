...
public class GenerateInviteCodeHandler implements RequestHandler<HttpRequest, HttpResponse> {
	Context context;
	public GenerateInviteCodeHandler(Context context) {
		super();
    ...
    public void onRequestFinish(HttpRequest request, HttpResponse response) {
        ...
        Log.i("invite code1", new String((byte[]) response.result()));
        if (commonMsg == null) {
            Toast.makeText(context, "请检查网络", Toast.LENGTH_SHORT).show();
            return;
        }
        if (!ResultCode.dealError(context, commonMsg.getResultCode())) {
            ...
            String registerCode = (String) read.get("inviteCode");
            Log.i("invite code", "invitecode=" + registerCode);
            User.getInstance().setBuiltRegCode(registerCode);
            User.getInstance().saveToCloud(context);
        }
    }
